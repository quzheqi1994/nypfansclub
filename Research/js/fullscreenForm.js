					...
					case 'div' : 
						[].slice.call( fld.querySelectorAll( 'ul > li' ) ).forEach( function( inp ) {
							inp.addEventListener( 'click', function(ev) {
							    var next;
                                switch(ev.srcElement.children[0].innerText){
                                    case "手机壳":
                                        next = self.current+1;
        							    var x = document.getElementsByClassName("li-hidden");
        							    while (x.length>0) {
                                            x[0].classList.add("li-show");
                                            x[0].classList.remove("li-hidden");
                                        }
                                    break;
                                    case "绳袋":
                                    case "毛巾":
                                        next = self.current+3;
        							    var x = document.getElementsByClassName("li-show");
        							    while (x.length>0) {
                                            x[0].classList.add("li-hidden");
                                            x[0].classList.remove("li-show");
                                        }
                                }
							    self._nextField(next);
							} );
						} ); 
						break;
				}
